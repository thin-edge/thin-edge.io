...
impl FileConfigPlugin {
    pub fn new(config: PluginConfig, use_tedge_write: TedgeWriteStatus) -> Self {
        Self {
            config,
            use_tedge_write,
        }
    ...
    pub fn set(&self, config_type: &str, from: &Utf8Path) -> Result<(), PluginError> {
        let entry = self
            .config
            .get_file_entry(config_type)
            .ok_or_else(|| PluginError::InvalidConfigType(config_type.to_string()))?;
...
"#;
        ...
        let plugin = FileConfigPlugin::new(config, TedgeWriteStatus::Disabled);
...
"#;
        ...
        let plugin = FileConfigPlugin::new(config, TedgeWriteStatus::Disabled);
        let result = plugin.set("unknown.conf", source_file.path().try_into().unwrap());

        assert!(matches!(result, Err(PluginError::InvalidConfigType(_))));
    }

    #[test]
    fn test_set_creates_file_successfully() {
        let ttd = TempTedgeDir::new();

        // Create source file
        let expected_content = "key=value\nfoo=bar\n";
        let source_file = ttd.file("source.conf").with_raw_content(expected_content);

        // Set up destination
        let dest_file = ttd.path().join("dest.conf");
...
"#,
        ...
        let plugin = FileConfigPlugin::new(config, TedgeWriteStatus::Disabled);
        let result = plugin.set("test.conf", source_file.path().try_into().unwrap());

        assert!(result.is_ok());
        assert!(dest_file.exists());

        let actual_content = std::fs::read_to_string(&dest_file).unwrap();
    ...
    fn test_set_creates_parent_directories() {
        ...
        // Create source file
        let expected_content = "nested=config\n";
        let source_file = ttd.file("source.conf").with_raw_content(expected_content);

        // Set up destination with nested path
        let dest_file = ttd.path().join("nested/subdir/dest.conf");
...
"#,
        ...
        let plugin = FileConfigPlugin::new(config, TedgeWriteStatus::Disabled);
        let result = plugin.set("nested.conf", source_file.path().try_into().unwrap());
