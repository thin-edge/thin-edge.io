...
mod tests {
    ...
    fn execute_with_defaults(command: &str) -> std::io::Result<ShellOutcome> {
        execute(sh(), command, tmp(), NO_LIMIT, NO_TIMEOUT)
    }
    ...
    fn a_large_output_is_truncated() {
        let outcome = execute(sh(), "printf '0123456789'", tmp(), 4, NO_TIMEOUT).unwrap();
        assert_eq!(outcome.exit_code, 0);
        assert_eq!(
            outcome.result,
    ...
    fn an_output_at_the_limit_is_not_truncated() {
        let outcome = execute(sh(), "printf '0123'", tmp(), 4, NO_TIMEOUT).unwrap();
        assert_eq!(outcome.exit_code, 0);
    ...
    fn a_character_split_by_the_truncation_is_dropped() {
        ...
        let outcome = execute(sh(), "printf 'éé'", tmp(), 3, NO_TIMEOUT).unwrap();
        assert_eq!(
            outcome.result,
    ...
    fn the_processes_started_by_a_command_are_terminated_on_timeout() {
        ...
        let outcome = execute(sh(), &command, tmp(), NO_LIMIT, Duration::from_millis(200)).unwrap();
        assert_eq!(outcome.timed_out, Some(Duration::from_millis(200)));
    ...
    fn the_truncation_notice_is_not_used_as_the_failure_reason() {
        let outcome = execute(sh(), "echo the real error; exit 3", tmp(), 20, NO_TIMEOUT).unwrap();
        let mut out = Vec::new();
        write_script_output(&mut out, &outcome).unwrap();
        assert!(String::from_utf8(out)
            .unwrap()
