...
/// before returning to the caller. In order to be able to wait for such an acknowledgement,
/// `Client#publish` first has to get informed about the `pkid` generated by the underlying
/// `rumqttc` library (via the `AckEvent::MessagePublish`). Depending on the QoS level, it then can
/// wait for the `AckEvent::PubAckReceived` (QoS=1) or `AckEvent::PubCompReceived` (QoS=2) event.
///
/// XXX WARNING WARNING WARNING XXX
///
/// This is not really reliable and has to be fixed by patching `rumqttc`.
///
/// WHY IS IT NOT RELIABLE?
///
/// There is a potential race between an emitted `Outgoing::Publish` event that is the direct
/// result of sending a `Publish` message to `rumqttc` and either an incoming `PubComp` or `PubAck`
/// message with a pkid that triggers a collision.  The collision will result in resending a
/// Publish message and that will emit another `Outgoing::Publish` message. This means that we
/// cannot reliably associate a message in `Client#publish` with an `pkid`. But we need that `pkid`
/// to wait for the acknowledgement of the publish message in QoS=1 or 2.
///
/// HOW TO FIX?
///
/// Required changes to `rumqttc`:
///
/// Change `Request::Publish` in `rumqttc` to take a correlation id and associate that with the
/// Publish message. Emit a `PublishAcknowledged` event containing this correlation id once we come
/// to the conclusion that the publish succeeded (depending on the QoS, either immediatly or upon
/// `PubAck` or `PubRel`).
///
/// In `mqtt_client::Client#publish`: Allocate a fresh correlation id (using atomic fetch and add)
/// and attach it to the `Request::Publish`. Then wait for the `PublishAcknowledged` event for that
/// correlation id.
///
...
impl Client {
    ...
    ///
    /// NOTE: This method has been changed from taking `&self` to `&mut self` to prevent
    /// anyone from trying to publish at the same time which would potentially lead to a races on
    /// the AckStream. Once we have a better way of associating a `message` with a `pkid` (for the
    /// purpose of waiting for the acknowledgement) we can drop that restriction again.
    pub async fn publish(&mut self, message: Message) -> Result<(), Error> {
        ...
        // Wait for a confirmation from the `rumqttc` backend that the message has been published.
    ...
    async fn bg_process(
        ...
        loop {
            match event_loop.poll().await {
                ...
                // XXX WARNING XXX
                //
                // See the big warning on the description of `AckEvent`.
                Ok(Event::Outgoing(Outgoing::Publish(pkid))) => {
                    send_discarding_error!(ack_event_sender, AckEvent::MessagePublished(pkid));
                }
...
impl AckEventStream {
    ...
    /// Waits for the next `AckEvent::MessagePublished` event and returns it's `pkid`.
    async fn wait_for_message_published(&mut self) -> Result<MessageId, Error> {
        loop {
            match self.receiver.recv().await? {
                AckEvent::MessagePublished(pkid) => return Ok(pkid),
                _ => {}
            }
        }
    }
