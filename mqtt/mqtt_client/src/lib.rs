...
impl Client {
    ...
    pub async fn connect(name: &str, config: &Config) -> Result<Client, Error> {
        ...
        let (mqtt_client, eventloop) = rumqttc::AsyncClient::new(mqtt_options, config.queue_capacity);
    ...
    /// }
    /// ```
    pub async fn publish_with_ack(&self, message: Message) -> Result<impl Future<Output = Result<(), Error>>, Error> {
        let qos = message.qos;
    ...
    pub async fn subscribe(&self, filter: TopicFilter) -> Result<MessageStream, Error> {
        let () = self.mqtt_client.subscribe(&filter.pattern, filter.qos).await?;
        ...
        loop {
            match self.receiver.recv().await {
                ...
                Err(broadcast::error::RecvError::Lagged(lag)) => {
                    ...
                    send_discarding_error!(self.error_sender, Arc::new(Error::MessagesSkipped { lag }));
                    continue;
                }
            }
        }
...
impl ErrorStream {
    ...
    pub async fn next(&mut self) -> Option<Arc<Error>> {
        match self.receiver.recv().await {
            ...
            Err(broadcast::error::RecvError::Lagged(lag)) => Some(Arc::new(Error::ErrorsSkipped { lag })),
        }
    }
}
