...
async fn publish(config: &Config, qos: QoS, payload: impl Into<String>) -> Result<(), mqtt_client::Error> {
...
async fn pipelined_publish(config: &Config, payload: impl Into<String>) -> Result<(), mqtt_client::Error> {
    let topic = Topic::new("test/uubpb9wyi9asi46l624f")?;
    let client = config.connect("publisher").await?;
