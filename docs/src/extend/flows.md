...
└── params.toml.template  # (optional) Configuration defaults; copy to params.toml to customise
```
...
## Example 1: Remapping sensor data
...
## Example 3: Configurable alarm threshold
...
- **`context.flow`** — a key-value store shared between all scripts within the same flow, useful for inter-step coordination
...
**debounce.js** maintains a counter in `context.flow`. The counter is incremented on each above-threshold reading and reset on a normal reading. An alarm is emitted only once the counter reaches `min_consecutive`.
...
export function onMessage(message, context) {
    ...
    if (payload._above_threshold) {
        const count = (context.flow.get("consecutive_count") ?? 0) + 1;
        context.flow.set("consecutive_count", count);
        ...
        context.flow.set("consecutive_count", 0);
        return [{
            topic: alarmTopic,
            payload: "",
            mqtt: { retain: true, qos: 1 },
...
Once a flow is installed (under `/etc/tedge/flows/`), you can test it without `--flows-dir`. The command uses flows already loaded by the running mapper:
...
The %%te%% runtime executes scripts as ECMAScript Modules (ES2020). You can author flows in TypeScript and use any bundler (esbuild, Rollup, webpack) to transpile and bundle them into a single `.js` file.

The [`tedge-flows-examples`](https://github.com/thin-edge/tedge-flows-examples) repository uses **esbuild** via **npm** as its build toolchain. Each flow is written in TypeScript with Jest unit tests, and the build step produces a bundled ECMAScript Module ready to package.
...
```sh
cd dist
COPYFILE_DISABLE=1 tar czf ../my-flow.tar.gz flow.toml main.js params.toml.template
```
...
A repository of maintained example flows, each with TypeScript source, npm build tooling, Jest unit tests, and packaging scripts. Covers a wide range of use-cases:
...
| [Builtin mapping rules](../references/mappers/builtin-flows.md) | The built-in c8y/az/aws mapper flows that can be customised or extended |
