...
Operation specific arguments can be provided to the sub-command instance,
and data extracted from the calling command status topic and message payload can be injected into these arguments.

- For instance, `cmd_arg.url = "http://127.0.0.1:8000/tedge/file-transfer/foo.txt"`
  adds an `url` property to the init message of the sub-command.
- Similarly, `cmd_arg.url = "${.payload.config_update.url}"`
  adds an `url` property to the init message of the sub-command, but extracting this url from the calling command state.
- Several such arguments can be added to the sub-command initial state.
  - All these individual arguments are combined into a JSON object value published on the sub-command MQTT topic.
  - The sub-command initial state can be provided as a whole with `cmd_arg = ${.payload.path.to.sub-command.init-status}`
  ...
  - If a `cmd_arg.status` value is provided, this value will be ignored and replaced by `"init"`
    as this is the only valid value for the initial state of a command.
  - If no value at all is provided for `cmd_arg`, then the default value is `{"status" = "init"}`.
- There is no way to assign a command identifier for the new sub-command instance.
  - This identifier is generated by %%te%%, which creates, monitors and clears the MQTT topic for the sub-command.

From an execution perspective, triggering a sub-command is similar to running a background script.

- The calling command workflow is paused while the sub-command is processed.
- The calling workflow move to an `on_exec` state from where to sub-command execution is watched till success, failure or timeout.
- A specific `"await-command-completion"` action is to be used for this `on_exec` state where the sub-command completion is awaited.
- The steps following the sub-command completion are defined on the `on_exec` state with `on_success`, `on_error` and `on_timeout` handlers.
...
- `await-command-completion` awaits for a sub-command to reach a success, failure or timeout
...
When a sub `command` is executed, the operation workflow moves to a waiting state which `action` must be `await-command-completion`.
This waiting state specifies, with `on_success`, `on_error` and `on_timeout` handlers,
the next step of the calling workflow on success, failure or timeout of the sub-command.

The `await-command-completion` action also tells how to inject back the outcome of the sub-command into the invoking command state.
- This is done by attaching values to the `payload` subfields, as in `payload.x.y.z = "${.command.payload.x.y.z}"`
...
- The payload as a whole can be overridden, as in `payload = "${.command.payload}`, however this is discouraged.
- Specific path expressions can be used to extract specific excerpts from the sub-command final state.
- `${.command}` is a json for the whole message including the `topic` and the `payload`.
- `${.command.topic}` is the sub-command request topic
- `${.command.topic.target}` is the sub command target identity
- `${.command.topic.operation}` is the command operation
- `${.command.topic.cmd_id}` is the command request unique identifier
- `${.command.payload}` is the whole sub command json payload
- `${.command.payload.status}` is the sub command final status
- `${.command.payload.x.y.z}` is the json value extracted from the payload following the given `x.y.z` path if any.
- If given `${.some.unknown.path}`, the subfield is assigned the unchanged expression.

```toml
[waiting_for_config_update]
action = "await-command-completion"
payload.x.y.z = "${.command.payload.x.y.z}"
timeout_second = 600
on_timeout = "timeout_config_update"
on_success = "successful_config_update"
on_error = { status = "failed", reason = "fail to update the config"}
