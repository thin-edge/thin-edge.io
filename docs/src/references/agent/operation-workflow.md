...
- Similarly, `input.url = "${.payload.config_update.url}"`
  adds an `url` property to the init message of the sub-command, extracting this url from the calling command state.
...
- For cases where there no one-to-one relationship between the properties of the current command state
  and the properties of sub-command init state, the sub-command init state can be computed using a script.
  - `input_script = "/some/script/which/stdout/is/used/as/sub-command/init/state"`
  ...
  - When the input script is successful, its stdout is used to determine the init state of the sub-command
      - From this output, only the excerpt between a `:::begin-tedge:::` header and a `:::end-tedge:::` trailer is decoded.
      - If this excerpt is a json payload, this payload is injected into the previous message payload
        (adding new fields, overriding overlapping ones, keeping previous unchanged ones).
...
- `${.}` denotes the whole sub-command final message including the `topic` and the `payload`.
- `${.topic}` is the sub-command request topic
