...
Register `device/child0//` as a child device of the main device(`device/main//`):
...
Register a `device/child0/service/nodered` as a service of `device/child0//`:
...
The entity metadata definition for an entity can be retrieved by subscribing to its topic id.

**Example: Fetch metadata of an entity**

Fetch the metadata of `device/child01//`:

```sh te2mqtt formats=v1
tedge mqtt sub 'te/device/child01//' --count 1
```
...
**Example: Fetch metadata of all entity**
