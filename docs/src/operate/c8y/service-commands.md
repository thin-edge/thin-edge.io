...
description: Starting, stopping and restarting a service from Cumulocity
---
...
## Restrictions on core %%te%% components

The workflows %%te%% ships refuse to stop `tedge-agent` and `tedge-mapper-c8y`
because both must stay alive to complete a service command.
See [services that cannot be stopped](../../references/agent/service-commands.md#services-that-cannot-be-stopped).

Restarting `tedge-agent` and `tedge-mapper-c8y` from Cumulocity works,
and the operation completes once the service is back.
