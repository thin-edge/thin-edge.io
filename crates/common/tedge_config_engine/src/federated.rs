...
use crate::defaults::validate_root_dependencies;
use crate::defaults::RootResolver;
use crate::ops::Action;
use crate::ops::ConfigOps;
...
impl FederatedConfig {
    ...
    pub fn mount(&mut self, prefix: &str, source: Box<dyn ConfigOps>) -> Result<(), ConfigError> {
        if self.mounts.iter().any(|m| m.prefix == prefix) {
            return Err(ConfigError::DuplicatePrefix(prefix.to_owned()));
        }
    ...
    fn route_mut(&mut self, key: &str) -> Result<(&mut ConfigMount, String), ConfigError> {
        // Index lookup avoids computing the error (which hits the filesystem) on success,
        // and sidesteps a mutable-vs-shared borrow conflict a direct loop would cause
        let found = self.mounts.iter().enumerate().find_map(|(i, mount)| {
            let local_key = key.strip_prefix(&mount.prefix)?.to_owned();
            Some((i, local_key))
        });
        match found {
            Some((i, local)) => Ok((&mut self.mounts[i], local)),
            None => Err(make_unknown_key_error(key, &self.config_dir, &self.mounts)),
        }
    }
