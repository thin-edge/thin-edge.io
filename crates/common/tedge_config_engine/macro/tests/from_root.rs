...
mod reading_from_root_keys {
    ...
    fn reader_field_resolves_through_the_supplied_root_config() {
        let mgr = mapper::manager();
        let dto = mapper::MapperConfigDto::default();
        let config: mapper::MapperConfig = mgr
            .build_reader(&dto, Some(&root_with_cert), "", None)
            .unwrap();
        assert_eq!(
    ...
    #[test]
    fn from_root_defaults_require_a_root_config_to_build_the_reader() {
        let mgr = mapper::manager();
        let dto = mapper::MapperConfigDto::default();
        let err = mgr
...
mod mounting_into_a_federated_config {
    ...
    fn root_config_cannot_use_from_root_defaults() {
        ...
        assert_eq!(
            err.to_string(),
            "'device.cert_path' uses a from_root default ('device.cert_path'), \
             which is not allowed in the root config itself"
        );
    }
