use rustls_022::pki_types::pem::PemObject;
use rustls_022::pki_types::CertificateDer;
use rustls_022::pki_types::PrivateKeyDer;
use rustls_022::ClientConfig;
use rustls_022::RootCertStore;
use rustls_pemfile_022::certs;
    ...
    P: AsRef<Path>,
{
    ...
    for cert in rustls_native_certs_022::load_native_certs().expect("could not load platform certs")
    {
...
pub fn read_pvt_key(
    ...
    PrivateKeyDer::from_pem_file(&key_file).map_err(|err| {
        if let rustls_022::pki_types::pem::Error::NoItemsFound = err {
            CertificateError::UnknownPrivateKeyFormat
        } else {
            CertificateError::CertificateParseFailed {
...
pub fn read_cert_chain(
    ...
    certs.map_err(|e| CertificateError::CertificateParseFailed {
...
mod tests {
    ...
    fn parse_private_rsa_key() {
        ...
        let result = read_pvt_key(temp_file.path()).unwrap();
        let pvt_key = vec![48, 46, 2, 1];
        assert_eq!(result.secret_der(), pvt_key);
    }
    ...
    fn parse_supported_key() {
        ...
        let expected_pvt_key = vec![48, 46, 2, 1];
        assert_eq!(parsed_key.secret_der(), expected_pvt_key);
    }
