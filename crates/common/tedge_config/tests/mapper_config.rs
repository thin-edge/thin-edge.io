...
async fn mapper_config_is_created_if_tedge_toml_does_not_exist() {
    ...
    tedge_config
        .update_toml(&|dto, _rdr| {
            dto.c8y.try_get_mut(None, "c8y").unwrap().url = Some("example.com".parse().unwrap());
            Ok(())
        })
        .await
        .unwrap();
    ...
    let tedge_toml = tokio::fs::read_to_string(ttd.path().join("tedge.toml"))
        .await
        .unwrap();
    let c8y_mapper_toml = tokio::fs::read_to_string(ttd.path().join("mappers/c8y/mapper.toml"))
        .await
        .unwrap();
