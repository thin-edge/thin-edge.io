...
use tedge_utils::fs_notify::{fs_notify_stream, pin_mut, FileEvent, Watch};
...
impl Mapper {
    ...
    pub(crate) async fn run(&mut self, ops_dir: Option<&Path>) -> Result<(), MqttError> {
        info!("Running");
    ...
    async fn process_messages(&mut self, ops_dir: Option<&Path>) -> Result<(), MqttError> {
