...
impl From<BridgeConfigC8yParams> for BridgeConfig {
    fn from(params: BridgeConfigC8yParams) -> Self {
        ...
        if use_mqtt_service {
            ...
            topics.push(format!(r#"# out 1 {topic_prefix}/custom/out/ """#));

            // Custom incoming topics subscribed on MQTT service
            for topic in custom_topics.0.iter() {
                topics.push(format!(r#"{topic} in 1 {topic_prefix}/custom/in/ """#));
            }
        }
    ...
    fn test_bridge_config_mqtt_service_enabled() -> anyhow::Result<()> {
        ...
        let expected = BridgeConfig {
            ...
            topics: vec![
                ...
                r#"# out 1 c8y/custom/out/ """#.into(),
                // MQTT service custom incoming topics
                r#"test in 1 c8y/custom/in/ """#.into(),
                r#"demo in 1 c8y/custom/in/ """#.into(),
            ],
            try_private: false,
            start_type: "automatic".into(),
            clean_session: true,
