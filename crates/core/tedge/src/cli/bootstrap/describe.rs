...
fn render_cloud(out: &mut String, descriptor: &CloudDescriptor) {
    ...
    let url = match &descriptor.url {
        Some(spec) if spec.pinned_value().is_some() => {
            format!("{} (not asked)", spec.pinned_value().unwrap())
        }
        Some(spec) => {
...
fn render_setting(out: &mut String, descriptor: &CloudDescriptor, setting: &Setting) {
    ...
    match (&setting.default, setting.prompt) {
        (Some(value), false) => notes.push(format!("{value}, not asked")),
        (Some(default), true) => notes.push(format!("default: {default}")),
        (None, _) => {}
    }
    let notes = match notes.is_empty() {
        true => String::new(),
