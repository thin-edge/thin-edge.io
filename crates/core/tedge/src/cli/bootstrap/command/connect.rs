...
impl BootstrapCommand {
    ...
    pub(super) async fn offline_staging_blocker(&self) -> Option<&'static str> {
        ...
        let has_identity =
            self.device_id.is_some() || read_config_string(&config, key::DEVICE_ID).is_some();
        if !has_identity {
            return Some(
    ...
    /// `staging_blocker` is why an offline run cannot even stage the services
    pub(super) async fn connect(
        &self,
        staging_blocker: Option<&'static str>,
        ...
        // and network exist
        if let Some(reason) = staging_blocker {
            self.ui
