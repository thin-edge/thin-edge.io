...
use super::c8y;
use super::hooks::read_dir_if_present;
use super::resolve::HOOK_METHOD;
use anyhow::bail;
use anyhow::Context;
...
pub struct CloudDescriptor {
    ...
    /// How the cloud URL is obtained (prompt text, default, or a value that is not asked for)
    #[serde(default)]
    pub url: Option<UrlSpec>,
    /// Settings the cloud needs (beyond the URL);
    /// a setting declared `prompt = false` is applied without asking
    #[serde(default)]
...
pub struct UrlSpec {
    ...
    /// Whether the wizard asks for the URL (the default).
    /// With `prompt = false`, the default is used without asking,
    /// and non-interactive runs do not require --url.
...
impl UrlSpec {
    /// The value to use without asking, when the URL is declared `prompt = false`
    pub fn pinned_value(&self) -> Option<&str> {
        (!self.prompt).then_some(self.default.as_deref()).flatten()
    }
}
...
pub struct Setting {
    ...
    /// Wizard-only unless `prompt = false`: shipped configuration defaults
    /// belong in the mapper's own `mapper.toml`, not in the descriptor
    #[serde(default)]
    pub default: Option<String>,
    /// Whether the wizard asks for this setting (the default).
    /// With `prompt = false`, the default is applied without asking,
    /// in interactive and non-interactive runs alike -
    /// the way a cloud pins a config value (e.g. a derived cloud
    /// pinning its transport). Method-level and choice-level `set` values
    /// and explicit `--set` values override it. Requires a default
    #[serde(default = "default_true")]
    pub prompt: bool,
    /// When set, the wizard offers these values as a pick-list.
    ///
...
impl Setting {
    /// The value to apply without asking, when the setting is declared `prompt = false`
    pub fn pinned_value(&self) -> Option<&str> {
        (!self.prompt).then_some(self.default.as_deref()).flatten()
    }
}
...
impl CloudDescriptor {
    /// The descriptor of a cloud none was declared for
    /// (a custom mapper registering via its hooks): generic prompts, no methods
    pub fn undeclared(cloud: &str) -> Self {
        Self {
            cloud: cloud.to_owned(),
            cloud_type: None,
            description: cloud.to_owned(),
            register: Vec::new(),
            url: None,
            settings: Vec::new(),
            hidden: false,
    ...
    /// Reject declarations that would be silently ignored
    pub fn validate(&self) -> anyhow::Result<()> {
        if let Some(url) = &self.url {
            if !url.prompt && url.default.is_none() {
                bail!("[url] declares prompt = false without a default");
            }
        }
        for setting in &self.settings {
            if !setting.prompt && setting.default.is_none() {
                bail!(
                    "[[settings]] {:?} declares prompt = false without a default",
                    setting.key
                );
            }
            if let (Some(default), false) = (&setting.default, setting.choices.is_empty()) {
                if !setting
                    .choices
                    .iter()
                ...
                {
                    bail!(
                        "[[settings]] {:?} declares a default that is not one of its choices",
                        setting.key
                    );
                }
            }
...
/// naming the methods and inputs the built-in implementations act on
...
name = "security token"
env = "{security_token}"
description = "Entered by the operator when accepting the registration, on tenants that demand one; generated when empty"
required = false

[[clouds.register]]
name = "{basic_preregistered}"
...
"#,
        c8y = c8y::CLOUD,
        c8y_ca = c8y::method::CA,
        self_signed = c8y::method::SELF_SIGNED,
        basic = c8y::method::BASIC,
        basic_preregistered = c8y::method::BASIC_PREREGISTERED,
        one_time_password = c8y::env::ONE_TIME_PASSWORD,
        c8y_user = c8y::env::USER,
        c8y_password = c8y::env::PASSWORD,
        bootstrap_user = c8y::env::BOOTSTRAP_USER,
        bootstrap_password = c8y::env::BOOTSTRAP_PASSWORD,
        security_token = c8y::env::SECURITY_TOKEN,
        device_user = c8y::env::DEVICE_USER,
        device_password = c8y::env::DEVICE_PASSWORD,
        hook = HOOK_METHOD,
    );
...
pub async fn load_descriptors(
    ...
    for dir in plugin_paths.iter().map(|root| root.join("clouds.d")) {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if let Some(marked) = name.strip_suffix(".ignore") {
                ...
                layer_hidden.insert(cloud.to_owned(), true);
                continue;
            }
            let Some(cloud) = name.strip_suffix(".toml") else {
                continue;
            };
            ...
            let descriptor: CloudDescriptor = toml::from_str(&content)
                .with_context(|| format!("Invalid cloud descriptor {path}"))?;
            descriptor
                .validate()
                .with_context(|| format!("Invalid cloud descriptor {path}"))?;
            // the file name is the cloud's address for markers and overrides
            if descriptor.cloud != cloud {
                bail!("Invalid cloud descriptor {path}: it declares the cloud {:?} but is named after {cloud:?}", descriptor.cloud);
            }
            // a marker in the same layer wins over its sibling descriptor
            layer_hidden.entry(cloud.to_owned()).or_insert(false);
            by_cloud.entry(cloud.to_owned()).or_insert(descriptor);
        }
    ...
    restore_builtin_method_inputs(&mut by_cloud, &builtins);

    // the built-in clouds first, in their compiled-in order, then the rest by name
    let mut descriptors: Vec<CloudDescriptor> = builtins
        .iter()
        .filter_map(|builtin| by_cloud.remove(&builtin.cloud))
        .collect();
    descriptors.extend(by_cloud.into_values());
    Ok(descriptors)
}

/// The built-in method implementations carry their inputs with them.
///
/// A descriptor with Cumulocity semantics restating a built-in method
/// without declaring inputs keeps the compiled-in inputs:
...
/// A descriptor declaring its own inputs for a built-in method wins.
fn restore_builtin_method_inputs(
    by_cloud: &mut BTreeMap<String, CloudDescriptor>,
    builtins: &[CloudDescriptor],
) {
    let Some(builtin_c8y) = builtins.iter().find(|d| d.cloud == c8y::CLOUD) else {
        return;
    };
    let c8y_semantics = |descriptor: &CloudDescriptor| {
        descriptor.cloud == c8y::CLOUD || descriptor.cloud_type.as_deref() == Some(c8y::CLOUD)
    };
    for descriptor in by_cloud.values_mut().filter(|d| c8y_semantics(d)) {
        for method in &mut descriptor.register {
...
/// A method input's value: as collected (or defaulted) for the run,
/// else from the process environment
pub fn input_value(collected: &[(String, String)], name: &str) -> Option<String> {
    collected
        .iter()
        .find(|(env, _)| env == name)
...
"#,
    ...
    #[tokio::test]
    async fn builtin_clouds_are_offered_first_and_in_order() {
        let tmp = tempfile::tempdir().unwrap();
        let root = camino::Utf8Path::from_path(tmp.path()).unwrap().to_owned();
        std::fs::create_dir_all(root.join("clouds.d")).unwrap();
        std::fs::write(root.join("clouds.d/acme.toml"), "cloud = \"acme\"").unwrap();
        // a descriptor is addressed by its file name
        std::fs::write(root.join("clouds.d/other.toml"), "cloud = \"mismatch\"").unwrap();
        let err = load_descriptors(std::slice::from_ref(&root))
            .await
            .unwrap_err();
        assert!(err.to_string().contains("named after"), "{err}");
        std::fs::remove_file(root.join("clouds.d/other.toml")).unwrap();

        let clouds: Vec<String> = load_descriptors(&[root])
            .await
            .unwrap()
            .into_iter()
            .map(|d| d.cloud)
            .collect();
        assert_eq!(clouds, ["c8y", "az", "aws", "acme"]);
    }
...
"#,
    ...
    #[test]
    fn unprompted_url_uses_its_default() {
        let pinned: UrlSpec = toml::from_str(
            r#"default = "iot.example.com"
prompt = false"#,
        )
        .unwrap();
        assert_eq!(pinned.pinned_value(), Some("iot.example.com"));
        let prompted: UrlSpec = toml::from_str(r#"default = "iot.example.com""#).unwrap();
        assert!(prompted.prompt);
        assert_eq!(prompted.pinned_value(), None);
    }

    #[test]
    fn prompt_false_requires_a_default() {
        let url: CloudDescriptor = toml::from_str(
            r#"cloud = "acme"
[url]
prompt = false"#,
        )
        .unwrap();
        let err = url.validate().unwrap_err().to_string();
        assert!(
            err.contains("[url]") && err.contains("without a default"),
            "{err}"
        );

        let setting: CloudDescriptor = toml::from_str(
            r#"cloud = "acme"
[[settings]]
key = "transport.port"
prompt = false"#,
        ...
        .unwrap();
        let err = setting.validate().unwrap_err().to_string();
        assert!(
            err.contains("transport.port") && err.contains("without a default"),
            "{err}"
        );

        let stray_default: CloudDescriptor = toml::from_str(
            r#"cloud = "acme"
[[settings]]
key = "transport.port"
default = "9999"
choices = ["1883", "8883"]"#,
        ...
        .unwrap();
        let err = stray_default.validate().unwrap_err().to_string();
        assert!(err.contains("not one of its choices"), "{err}");

        let pinned: CloudDescriptor = toml::from_str(
            r#"cloud = "acme"
[url]
...
prompt = false"#,
        ...
        assert_eq!(pinned.settings[0].pinned_value(), Some("8883"));
    }
...
"#,
        ...
        // its own (pinned) settings stay its own: the base's are not merged in
        assert_eq!(resolved.settings.len(), 1);
        assert_eq!(resolved.settings[0].pinned_value(), Some("true"));
