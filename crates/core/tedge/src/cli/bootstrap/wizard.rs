...
pub fn run(
    ...
    let descriptor = match &seed.cloud {
        Some(name) => match descriptors.iter().find(|d| d.cloud == *name) {
            ...
            None => {
                fallback_descriptor = CloudDescriptor::undeclared(name);
                &fallback_descriptor
            }
        },
        None => {
    ...
    let url = match &descriptor.url {
        ...
        Some(spec) if spec.pinned_value().is_some() => {
            let url = spec.pinned_value().unwrap().to_owned();
            prompter.say(&format!("URL: {url} (set by the {cloud} cloud descriptor)"));
            Some(url)
        }
        Some(spec) => {
    ...
    for setting in &descriptor.settings {
        // A pinned setting (prompt = false) is applied without asking,
        // in every mode, by the command itself
        if !setting.prompt {
            continue;
        }
        ...
        settings.extend(prompt_setting(prompter, setting, &cloud, full_key, seed)?);
    }
...
/// Ask one of the cloud's settings; the answer — and, for a pick-list,
/// any further config the chosen entry implies — compiles to `--set` pairs
fn prompt_setting(
    prompter: &mut Prompter,
    ...
    if setting.choices.is_empty() {
        ...
        return match value {
            Some(value) => Ok(vec![KeyValue::new(full_key, value)]),
    ...
    let default_index = setting
        ...
        .unwrap_or(0);
    let choice = &setting.choices[prompter.choice(&labels, default_index)?];
    let mut settings = vec![KeyValue::new(full_key, choice.value())];
    for (key, value) in choice.implied_config().into_iter().flatten() {
        ...
        if !seed.set_keys.contains(&implied_key) {
            settings.push(KeyValue::new(implied_key, value));
        }
    }
    Ok(settings)
}
...
mod tests {
    ...
    fn the_full_wizard_compiles_answers_to_flags() {
        ...
        assert_eq!(
            answers.settings,
            vec![KeyValue::new("c8y.mqtt_service.enabled", "true")]
        );
    ...
    fn seeded_answers_skip_their_questions_and_inputs_come_from_the_environment() {
        ...
        // and the optional security token is left empty
        let (mut prompter, transcript) =
