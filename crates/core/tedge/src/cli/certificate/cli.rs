...
use crate::cli::hsm::CreateKeyArgs;
...
pub enum TEdgeCertCli {
    ...
    /// present on the token (see `tedge hsm create-key` command if you need to create it) and
    ...
    /// Deprecated alias for `tedge hsm create-key`.
    #[clap(hide = true)]
    CreateKeyHsm(CreateKeyArgs),
...
impl BuildCommand for TEdgeCertCli {
    async fn build_command(self, config: &TEdgeConfig) -> Result<Box<dyn Command>, ConfigError> {
        ...
        let cmd = match self {
            ...
            TEdgeCertCli::CreateKeyHsm(args) => {
                eprintln!(
                    "Warning: `tedge cert create-key-hsm` is deprecated, use `tedge hsm create-key` instead."
                );
            ...
            TEdgeCertCli::Upload(UploadCertCli::C8y {
                ...
                let cmd = c8y::UploadCertCmd {
                    ...
                    cloud_root_certs: config.cloud_root_certs().await?,
                    username,
                    password,
                };
                cmd.into_boxed()
...
async fn get_device_id(
    ...
    match (id, config.device_id(cloud.as_ref()).ok()) {
        ...
        (None, Some(config_id)) => Ok(config_id),
...
mod tests {
    ...
    async fn validate_get_device_id_returns_ok(
        input_id: Option<&str>,
    ...
    async fn validate_get_device_id_returns_err(
        input_id: Option<&str>,
        cloud_arg: Option<CloudArg>,
        toml: toml::Table,
    ) {
