...
use pad::PadStr;
use tedge_config::tedge_config_cli::new_tedge_config::*;

pub struct ListConfigCommand {
    pub is_all: bool,
    pub is_doc: bool,
    pub config: NewTEdgeConfig,
}
...
fn print_config_list(config: &NewTEdgeConfig, all: bool) -> Result<(), ConfigError> {
    let mut keys_without_values = Vec::new();
    for config_key in ReadableKey::iter() {
        match config.read(config_key).ok().flatten() {
            Some(value) => {
