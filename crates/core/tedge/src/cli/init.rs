...
impl TEdgeInitCmd {
    async fn initialize_tedge(&self, config: TEdgeConfig) -> anyhow::Result<()> {
        ...
        create_directory_and_update_ownership(config_dir.join("operations").join("c8y"), &permissions).await?;
        ...
        create_directory_and_update_ownership(config_dir.join("device-certs"), &permissions).await?;
        create_directory_and_update_ownership(config_dir.join("mappers"), &permissions).await?;
        create_directory_and_update_ownership(config_dir.join(".tedge-mapper-c8y"), &permissions).await?;
