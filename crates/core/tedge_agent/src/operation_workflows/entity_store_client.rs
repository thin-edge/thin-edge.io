...
//!
//! The entity store runs on the main device only, and is reached over HTTP at the same address
//! as the file transfer service. An agent running on the main device queries its own HTTP server;
//! an agent running on a child device queries the main device's, as the config and log managers
//! already do for file transfers.
...
/// How long a lookup waits for the entity store
///
/// Neither the HTTP actor nor its hyper client sets any timeout, and the lookup is awaited in the
/// workflow actor's main loop: a store that accepts the connection but never answers would stall
/// every command of the agent, not just the one being looked up. An agent on a child device
/// reaches the store over the network, so such a connection is a real case.
...
pub struct EntityStoreClient {
    /// The URL of the entities collection, e.g. `http://127.0.0.1:8000/te/v1/entities`
...
impl EntityStoreClient {
    ...
    /// The URL of a single entity
    ///
    /// The topic identifier is used as is, with all its four segments, empty ones included:
    /// the REST API pads a shorter path with empty segments anyway.
