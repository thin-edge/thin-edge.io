...
/// How a thin-edge service is deployed.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ServiceDeployment {
    /// Started by an init unit of its own, such as the `tedge-agent` systemd service.
    OwnUnit,

    /// Hosted by `tedge run all`, with no init unit of its own.
    Hosted,
}

/// Actions a thin-edge service publishes when it starts.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
const TEDGE_SERVICE_ACTIONS: &[&str] = &["restart"];
...
///
/// assert_eq!(service_actions("tedge-agent", OwnUnit).declared, ["restart"]);
/// assert_eq!(service_actions("tedge-mapper-c8y", OwnUnit).declared, ["restart"]);
///
...
/// assert_eq!(service_actions("tedge-mapper-c8y", Hosted).withdrawn, ["restart"]);
/// ```
pub fn service_actions(service_name: &str, deployment: ServiceDeployment) -> ServiceActions {
    match deployment {
        // A hosted mapper has no unit, so the default service actions must be withdrawn
        ServiceDeployment::Hosted if service_name.starts_with("tedge-mapper-") => ServiceActions {
            declared: &[],
            withdrawn: TEDGE_SERVICE_ACTIONS,
        },

        _ => ServiceActions::declaring(TEDGE_SERVICE_ACTIONS),
    }
}
...
mod tests {
    ...
    fn a_service_thin_edge_ships_declares_its_restart(name: &str) {
    ...
    #[test_case("tedge-mapper-collectd"; "a mapper connected to no cloud")]
    #[test_case("tedge-mapper-local"; "the local mapper")]
    fn a_hosted_mapper_withdraws_the_restart_it_would_declare(name: &str) {
        let actions = service_actions(name, ServiceDeployment::Hosted);
        assert!(actions.declared.is_empty());
        assert_eq!(actions.withdrawn, ["restart"]);
    }
}
