...
pub enum OperationAction {
    ...
    ///
    /// The sub-command is implied: at most one sub-command can run concurrently.
    ///
    /// ```toml
    ...
    /// Resume this command on sub-command completion
    ///
    /// This action is generated by the sub-command on completion or by the agent on timeout
    ResumeOnCommandCompletion,
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let str = match self {
            ...
            OperationAction::ResumeOnCommandCompletion => {
                "resume on sub-command completion".to_string()
            }
