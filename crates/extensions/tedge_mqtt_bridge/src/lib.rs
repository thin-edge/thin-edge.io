        ...
        match notification {
            Event::Incoming(Incoming::ConnAck(conn_ack)) => {
                ...
                if let Some(msg) = &reconnect_message {
                    let msg = msg.clone();
                    log_event!(name, "Bridge connection publishing reconnect message: {msg:?}");
...
mod tests {
    ...
    mod bridge {
        ...
        async fn does_not_publish_when_no_reconnect_message_configured() {
            ...
            assert!(matches!(
                bridge.cloud_client.next_action(),
                Err(_) // No actions logged
            ))
        }
        ...
        async fn publishes_reconnect_message_with_correct_qos_and_retain() {
            ...
            match published {
                Action::Publish(msg) => {
                    assert_eq!(msg.topic, "reconnect/status");
                    assert_eq!(msg.qos, QoS::ExactlyOnce);
                    assert_eq!(msg.retain, true);
                    assert_eq!(msg.payload, "reconnected".as_bytes());
                }
