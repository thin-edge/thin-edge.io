...
impl MqttBridgeActorBuilder {
    ...
    pub async fn new(
        ...
        if let Some(auth_pass_config) = auth_config.username_and_password() {
            local_config.set_credentials(
                auth_pass_config.username,
                auth_pass_config.password.to_string(),
            );
        }
