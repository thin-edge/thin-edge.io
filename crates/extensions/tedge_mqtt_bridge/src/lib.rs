...
impl MqttBridgeActorBuilder {
    ...
    /// Watches whether the bridge is subscribed to the topics it relays to the cloud
    ///
    /// While this is `false`, the bridge has no subscription on the local broker, so anything
    /// published on those topics is discarded rather than forwarded to the cloud. Components
    /// sharing this process can await it before publishing anything the cloud has to receive.
