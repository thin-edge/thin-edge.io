...
pub trait Plugin {
    ...
    async fn finalize(&self, config_type: &str) -> Result<(), ConfigManagementError>;
}
...
impl Plugin for ExternalPlugin {
    async fn list(
        ...
        warn!("Listing config types using plugin command: {}", command);
    ...
    async fn get(
        ...
        warn!("Fetching config using command: {}", command);
    ...
    async fn set(
        ...
        warn!("Setting config using command: {}", command);
        self.execute(command, None).await?;
    ...
    async fn finalize(&self, config_type: &str) -> Result<(), ConfigManagementError> {
        let mut command = self.command(FINALIZE)?;
        command.arg(config_type);

        warn!("Finalizing config using command: {}", command);
        self.execute(command, None).await?;
