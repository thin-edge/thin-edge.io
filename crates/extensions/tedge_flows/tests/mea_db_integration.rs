...
use tedge_flows::flow::{DateTime, Message, MessageSource};
use tedge_flows::{MeaDB, MessageProcessor};
...
async fn stores_message_and_retrieves_it_with_correct_timestamp() {
    ...
    let mut db: MeaDB = MeaDB::open(&db_path).await.expect("Failed to open database");
    ...
    let stored_messages = db.query_all(series).await.expect("Failed to query messages");
    assert_eq!(stored_messages.len(), 1, "stored_messages: {stored_messages:?}");
...
async fn drains_messages_older_than_cutoff_leaving_newer_ones() {
    ...
    db.store(series, old_timestamp, old_msg.clone()).await.unwrap();
    db.store(series, new_timestamp, new_msg.clone()).await.unwrap();
    db.store(series, future_timestamp, future_msg.clone()).await.unwrap();
    ...
    assert_eq!(drained[0].1.payload, r#"{"temp": 20.0}"#);
    assert_eq!(drained[1].0, new_timestamp);
    assert_eq!(drained[1].1.payload, r#"{"temp": 25.0}"#);
    ...
    assert_eq!(remaining[0].1.payload, r#"{"temp": 30.0}"#);
}
...
async fn isolates_messages_between_different_series() {
    ...
    let temp_msg = create_test_message("te/device/main///m/temperature", r#"{"temperature": 25.5}"#);
    let humidity_msg = create_test_message("te/device/main///m/humidity", r#"{"humidity": 60.0}"#);

    // Store in different series
    db.store(series_a, timestamp, temp_msg.clone()).await.unwrap();
    db.store(series_b, timestamp, humidity_msg.clone()).await.unwrap();
    ...
    assert_eq!(humidity_messages.len(), 1, "humidity_messages: {humidity_messages:?}");
    assert_eq!(temp_messages[0].1.payload, r#"{"temperature": 25.5}"#);
    assert_eq!(humidity_messages[0].1.payload, r#"{"humidity": 60.0}"#);
}
...
async fn processes_mqtt_message_through_js_and_outputs_to_database() {
    ...
    assert_eq!(processed_messages.len(), 1, "processed_messages: {processed_messages:?}");
...
async fn drains_database_and_processes_messages_through_js() {
    ...
    "#;
    std::fs::write(config_dir.join("input_test.toml"), toml_content).expect("Failed to write TOML config");
    ...
    assert_eq!(drained_messages.len(), 1, "drained_messages: {drained_messages:?}");
...
async fn only_drains_database_at_configured_frequency_intervals() {
    ...
    std::fs::write(config_dir.join("identity.js"), 
    ...
    assert_eq!(drain_results_interval.len(), 1, "drain_results_interval: {drain_results_interval:?}");

    // Test that draining doesn't happen at non-intervals
    let drain_results_non_interval = processor.drain_db(non_interval).await;
    assert_eq!(drain_results_non_interval.len(), 0, "drain_results_non_interval: {drain_results_non_interval:?}");
}
...
async fn drains_messages_older_than_max_age_retention_period() {
    ...
    std::fs::write(config_dir.join("filter.js"), 
        "export function onMessage(message) { return [message]; }")
        .expect("Failed to write JS file");
    ...
    processor.database.store("age-filtered-data", recent_timestamp, recent_msg).await.unwrap();
    processor.database.store("age-filtered-data", old_timestamp, old_msg).await.unwrap();
    processor.database.store("age-filtered-data", very_old_timestamp, very_old_msg).await.unwrap();
    ...
    assert_eq!(drained_messages[0].1.payload, r#"{"value": "very_old"}"#, "Should drain the very_old message (45s ago)");
}
...
async fn chains_mqtt_storage_drain_and_output_flows_end_to_end() {
    ...
    let input_message = create_test_message("te/device/main///m/sensor", r#"{"temperature": 23.5, "humidity": 55.0}"#);
    ...
    assert!(!storage_results.is_empty(), "storage_results: {storage_results:?}");
    ...
    assert!(!drain_results.is_empty(), "drain_results: {drain_results:?}");
    ...
    for (_, drain_result) in drain_results {
        if let Ok(drained_messages) = drain_result {
            for (timestamp, message) in drained_messages {
                ...
                assert!(!process_results.is_empty(), "process_results: {process_results:?}");
...
async fn processes_messages_only_from_matching_input_sources() {
    ...
    std::fs::write(config_dir.join("passthrough.js"), 
        "export function onMessage(message) { return [message]; }")
        .expect("Failed to write JS file");
    ...
    let db_config = r#"
        ...
        steps = [{ script = "passthrough.js" }]
    "#;
    std::fs::write(config_dir.join("db_flow.toml"), db_config)
        .expect("Failed to write DB config");
    ...
    let mqtt_flow_result = mqtt_results.iter()
        .find(|(flow_id, _)| flow_id.contains("mqtt_flow"));
    let db_flow_result = mea_db_results.iter()
