...
pub struct CumulocityConverter {
    ...
    /// The actions declared by the services of this device and its children
    ...
    async fn try_convert_data_message(
        ...
        match &channel {
            ...
            Channel::CommandMetadata { operation } => {
                // Every command capability of a service is an action of that service,
...
pub(crate) mod tests {
    ...
    async fn test_register_operation() {
        ...
        // A command capability of a service is an action of that service,
        // not an operation of its own
        let msgs = converter.convert(&operation_msg).await;
        assert_messages_matching(
            &msgs,
