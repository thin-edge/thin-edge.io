...
# The reason to use an auth token
...
def get_artifacts_for_runid(runid, run_number, token):
    ...
    with open(os.path.expanduser(f'{lake}/results_{run_number}_metadata.json'), 'w') as ofile:
...
def get_all_runs(token):
    # curl -H "Accept: application/vnd.github.v3+json" -u abelikt:$TOKEN -L https://api.github.com/repos/abelikt/thin-edge.io/actions/runs
...
for i in get_all_runs(token):
    ...
    for test_run in i:
        if test_run['name'] == 'system-test-workflow':
            ...
            with open(os.path.expanduser(f'{lake}/system_test_{run_number}_metadata.json'), 'w') as ofile:
                ofile.write(json.dumps(test_run, indent=4))
            print(f"Found System Test Run with id {test_run['id']} run number {run_number} workflow id {test_run['workflow_id']}")
