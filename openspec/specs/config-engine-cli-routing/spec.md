# Config Engine CLI Routing

## Purpose

Defines how `tedge config` CLI commands route `mappers.<name>.<key>` keys
to the `tedge_config_engine` `FederatedConfig`,
including get, set, unset, list, add/remove for list keys,
environment variable overrides, feature gating, and error diagnostics.
