...
Lets external processes — plugins, containerized components, child devices —
read a curated set of thin-edge configuration values without filesystem
access to `/etc/tedge` or the device certificate.
Each owning component (the agent, each mapper) publishes only the settings
it explicitly marks as exposable, as one retained MQTT JSON document per
service, and the agent serves the same values over a read-only HTTP API.
