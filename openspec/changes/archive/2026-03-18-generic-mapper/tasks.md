...
- [x] 9.3 Fix the `unrecognised_names_are_flagged` test in `mappers_dir.rs`: the comment "Typo: missing 's' in thingsboard" is wrong (the typo is an extra 'e' in `custom`); reduce to two cases — one with an unrecognised type prefix (`not-custom.thingsboard`) and one bare unrecognised name (`thingsboard`)
...
- [x] 10.4 Add a test that verifies `warn_unrecognised_mapper_dirs` actually emits warnings for unrecognised directories (the existing tests only verify no panic); use `tracing_test` or capture log output to assert that `not-custom.thingsboard` and `thingsboard` each trigger a warning
