...
printf "\n\n"
read -p 'Enter the path to your root certificates. On most Linux systems that is [/etc/ssl/certs]: ' path_root_cert
...
printf "\n\n"
read -r -p "Would you like to remove the old cert and create a new one? (y)Yes/(n)No: " response
case $response in
    [yY]) 
        printf "\nRemoving cert...."
        tedge cert remove
...
printf "\n\n"
read -r -p "Looks like tedge is already connected to Cumulocity. Would you like to reconnect? (y)Yes/(n)No: " response
case $response in
    [yY])  
...
case $response in
    [yY])  
        ...
        printf "\nYou should now see a new measurement in your device $device_id. Enjoy....\n"
        ;;
    *)  
        printf "\nOk bye, enjoy thingEdge.io!!!\n" 
        exit 
