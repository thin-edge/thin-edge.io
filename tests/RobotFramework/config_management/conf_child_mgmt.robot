...
Startup child device
    Child Connection
    ${rc}=    Execute Command    printf ${config} > c8y-configuration-plugin    return_stdout=False    return_rc=True   #Create config file on child device
    Should Be Equal    ${rc}    ${0}
    Write    curl -X PUT http://${PARENT_IP}:${HTTP_PORT}/tedge/file-transfer/${CHILD}/c8y-configuration-plugin \\   #Create config file from child to the parent
    ...
    Should Be Equal    ${rc}    ${0}
    Close Connection
Request snapshot from child device    #MQTT SUB: 'tedge mqtt sub tedge/${CHILD}/commands/req/config_snapshot'
