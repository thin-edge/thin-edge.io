...
Run tedge diag collect
    Execute Command    tedge diag collect --tarball-name default
...
Exit with 0 when a plugin is non applicable
    Execute Command    tedge config set diag.plugin_path /setup/diag-plugins
    Execute Command
...
Exit with 1 when a plugin exits with non-zero"
    Execute Command    tedge config set diag.plugin_path ["/setup/diag-plugins","/usr/share/tedge/diag-plugins"]
    Execute Command
    ...    printf '#!/bin/sh\nexit 1\n' > /setup/diag-plugins/99_error.sh && chmod +x /setup/diag-plugins/99_error.sh
    Execute Command
    ...    cmd=tedge diag collect --tarball-name tedge-diag-error
    ...    exp_exit_code=1

No tarball is created when there is no plugin
    Execute Command
    ...    tedge diag collect --tarball-name tedge-diag-no-plugin --plugin-dir /setup/diag-plugins
    ...    exp_exit_code=2
...
Custom Test Teardown
    Execute Command    tedge config unset diag.plugin_path
