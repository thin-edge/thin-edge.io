...
Extra security guard for root user
    Execute Command
    ...
    Execute Command
    ...    touch /setup/diag-plugins/writable_by_other_users.sh && chmod a+wx /setup/diag-plugins/writable_by_other_users.sh
    ${stderr}=    Execute Command
    ...    sudo tedge diag collect --plugin-dir /setup/diag-plugins
    ...    stdout=${False}
    ...    stderr=${True}
    ...    ignore_exit_code=${True}
    Should Contain    ${stderr}    (not owned by root)
    Should Contain    ${stderr}    (writable by non-root users)
