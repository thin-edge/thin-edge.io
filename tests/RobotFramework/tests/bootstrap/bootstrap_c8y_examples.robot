...
QR Code Is Rendered For The Pending Registration
    ...
    Install Example Files    qr-registration/configure.d/90_qr_code    configure.d
...
Full Bootstrap Runs The Event And Device Link Finalize Hooks
    Install Example Files    bootstrap-event/finalize.d/50_bootstrap_event    finalize.d
    Install Example Files    device-link/finalize.d/60_device_link    finalize.d
    ${domain}=    Cumulocity.Get Domain
    ${output}=    Execute Command
...
Bootstrap With The Cumulocity CA Is The Default Method
    ...
    ${output}=    Execute Command    cmd=tedge bootstrap c8y 2>&1
    Should Contain    ${output}    completed successfully
    ${log}=    Bootstrap Log
    Should Contain    ${log}    certificate already present
...
Bootstrap pre-registered basic auth device
    ...
    Execute Command    cmd=tedge bootstrap c8y
    ${log}=    Bootstrap Log
    Should Contain    ${log}    credentials already present
    ...
    ${output}=    Execute Command
    ...    cmd=sudo env C8Y_DEVICE_USER='${CREDENTIALS.username}' C8Y_DEVICE_PASSWORD='${CREDENTIALS.password}' tedge bootstrap c8y --re-register 2>&1
    Should Contain    ${output}    credentials stored at ${credentials_path}
    ${log}=    Bootstrap Log
    Should Contain    ${log}    removed ${credentials_path}
    Execute Command    cmd=tedge connect c8y --test
...
Clean Unwinds Only The Bootstrap-Managed Configuration
    ...
    ${output}=    Execute Command
    ...    cmd=sudo env C8Y_DEVICE_USER='${CREDENTIALS.username}' C8Y_DEVICE_PASSWORD='${CREDENTIALS.password}' tedge bootstrap c8y --clean --url "${domain}" --device-id "${DEVICE_SN}" --register basic-preregistered 2>&1
    Should Contain    ${output}    completed successfully
    ${log}=    Bootstrap Log
    Should Contain    ${log}    removed ${credentials_path}
    Should Contain    ${log}    unset the bootstrap-managed c8y.* keys
...
Bootstrap using a non-interactive service
    ...
    # bench run: save the fleet-wide invocation, applying nothing
    Execute Command
    ...
    # the per-device values are supplied by name, to be read from the env file
    Execute Command
