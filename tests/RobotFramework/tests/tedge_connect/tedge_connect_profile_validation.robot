...
Bridge topic prefix must be unique to test connection
    ThinEdgeIO.Execute Command    sudo tedge config set c8y.url --profile second example.com     timeout=0 
    Verify conflicting configuration error appears    sudo tedge connect c8y --test --profile second    bridge.topic_prefix

Device ID must be unique if cloud URLs are the same
    ThinEdgeIO.Execute Command    sudo tedge config set c8y.url --profile second "$(tedge config get c8y.url)"    timeout=0 
...
Proxy bind port must be unique
    ThinEdgeIO.Execute Command    sudo tedge config set c8y.url --profile second example.com    timeout=0 
    ThinEdgeIO.Execute Command    sudo tedge config set c8y.bridge.topic_prefix --profile second c8y-second    timeout=0 
