...
Consuming messages from a process command with a relative path argument
    [Documentation]    When [input.process] uses a relative path as a command argument, the cwd is
    ...    set to the flows directory so the path resolves correctly
    ThinEdgeIO.Transfer To Device
    ...    ${CURDIR}/relative-process-flows/relative-cwd-data.json
    ...    /etc/tedge/mappers/local/flows/relative-cwd-data.json
    Install Flow    relative-process-flows    relative-cwd.toml
    ${start}    Get Unix Timestamp
    ${messages}    Should Have MQTT Messages
    ...    topic=test/relative-cwd
    ...    minimum=1
    ...    date_from=${start}
    ...    message_contains=hello-from-relative-cwd
    [Teardown]    Run Keywords
    ...    Uninstall Flow    relative-cwd.toml
    ...    AND    Execute Command    cmd=rm -f /etc/tedge/mappers/local/flows/relative-cwd-data.json
