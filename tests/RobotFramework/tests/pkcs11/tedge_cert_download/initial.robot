...
Tedge cert download error
    ...
    ...    cmd=tedge cert download c8y --device-id "${DEVICE_SN}" --one-time-password '${credentials.one_time_password}' --retry-every 5s --max-timeout 60s    stdout=False    stderr=True    exp_exit_code=!0
    ...
    Should Contain    ${stderr}    2: tedge-p11-server wasn't able to understand the request, perhaps because its version is too old
