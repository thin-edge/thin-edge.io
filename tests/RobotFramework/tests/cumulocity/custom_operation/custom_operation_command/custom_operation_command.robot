...
Run shell custom operation for main device with custom topic
    ...
    Execute Command
    ...    tedge mqtt pub --retain '${prefix}/custom/topic/one' '{"status":"PENDING","id":"1234","c8y_Command":{"text":"echo helloworld3"},"externalSource":{"externalId":"${DEVICE_SN}","type":"c8y_Serial"}}'

    Should Have MQTT Messages
    ...    c8y/s/us
    ...    message_pattern=^(504|505|506),1234($|,\\"helloworld3\n\\")
    ...    minimum=2
    ...    maximum=2
