...
fn multi_value_translation() {
    ...
    let output = CumulocityJson::from_thin_edge_json(&String::from(multi_value_thin_edge_json).into_bytes());
    match output {
        Ok(vec) => {
            println!("{}", String::from_utf8(vec).unwrap());
        }
...
fn thin_edge_translation_with_type_and_time_stamp() {
    ...
    let output =
        CumulocityJson::from_thin_edge_json(&String::from(single_value_thin_edge_json_with_type_and_time).into_bytes());
    match output {
        Ok(vec) => {
            println!("{}", String::from_utf8(vec).unwrap());
        }
