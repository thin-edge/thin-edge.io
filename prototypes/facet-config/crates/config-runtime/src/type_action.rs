...
//! the underlying DTO fields are strongly typed (`u16`, `TemplatesSet`, etc.). Each
//! action (set, add, remove, …) needs type-aware logic — for instance, `add` on a
//! `TemplatesSet` merges into a set, while `add` on a `u16` just overwrites.
//!
//! Writing that dispatch by hand for every action × every type is repetitive: parse
...
use crate::reflect::{find_leaf_shape, ConfigError};
