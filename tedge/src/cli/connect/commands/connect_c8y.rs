...
pub struct ConnectC8yCommand {
    pub config_repository: TEdgeConfigRepository,
}

impl Command for ConnectC8yCommand {
    fn description(&self) -> String {
        "create bridge to connect Cumulocity cloud.".into()
    }

    fn execute(&self, context: &ExecutionContext) -> Result<(), anyhow::Error> {
        let mut config = self.config_repository.load()?;
        assign_default(&mut config, C8yRootCertPathSetting)?;
        let bridge_config = create_c8y_bridge_config(&config)?;
...
fn create_c8y_bridge_config(config: &TEdgeConfig) -> Result<BridgeConfig, ConfigError> {
    ...
    Ok(BridgeConfig::new_for_c8y(params))
}
