...
impl Connect {
    fn new_bridge(&self) -> Result<(), ConnectError> {
        ...
        println!("Restarting MQTT Server, [requires elevated permission], please authorise if asked.\n");
    ...
    fn clean_up(&self) -> Result<(), ConnectError> {
        ...
        let path = utils::build_path_from_home(&[TEDGE_HOME_PREFIX, TEDGE_BRIDGE_CONF_DIR_PATH, C8Y_CONFIG_FILENAME])?;
    ...
    fn write_bridge_config_to_file(&self, config: &BridgeConf) -> Result<(), ConnectError> {
        ...
        let dir_path = utils::build_path_from_home(&[TEDGE_HOME_PREFIX, TEDGE_BRIDGE_CONF_DIR_PATH])?;
        ...
        let config_path =
            utils::build_path_from_home(&[TEDGE_HOME_PREFIX, TEDGE_BRIDGE_CONF_DIR_PATH, C8Y_CONFIG_FILENAME])?;
...
impl Default for C8yConfig {
    fn default() -> Self {
        let cert_path = utils::build_path_from_home(&[TEDGE_HOME_PREFIX, DEVICE_CERT_NAME]).unwrap_or_default();

        let key_path = utils::build_path_from_home(&[TEDGE_HOME_PREFIX, DEVICE_KEY_NAME]).unwrap_or_default();
