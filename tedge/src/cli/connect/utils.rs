...
fn cmd_nullstdio_args(command: &str, args: &[&str], expected_code: i32) -> Result<bool, ConnectError> {
    Ok(std::process::Command::new(command)
        ...
        .status()
        .map_or_else(|err| Err(err), |status| Ok(status.code() == Some(expected_code)))?)
}
...
fn cmd_nullstdio_args_with_code_with_sudo(command: &str, args: &[&str]) -> Result<ExitStatus, ConnectError> {
    Ok(std::process::Command::new(command)
        .args(args)
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null())
