...
impl Command for MqttCmd {
    fn to_string(&self) -> String {
        match self {
            MqttCmd::Pub { topic, message, qos } => format!(
    ...
    fn run(&self, _verbose: u8) -> Result<(), anyhow::Error> {
        match self {
            MqttCmd::Pub { topic, message, qos } => publish(topic, message, *qos)?,
...
async fn publish(topic: &str, message: &str, qos: QoS) -> Result<(), MqttError> {
    let mqtt = Config::new(DEFAULT_HOST, DEFAULT_PORT).connect(DEFAULT_ID).await?;
...
mod tests {
    ...
    fn test_cli_pub_basic() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let assert = cmd.args(&["mqtt", "pub", "topic", "message"]).unwrap().assert();
